# Name: Seungwon Yang  Apr. 19, 2013
# Filename: topic_extraction_TFIDF_only.py
# Description: this is to compute TF*IDF-only topic tags from 
#              both ctr_30 and various_30 datasets.
#			   Also, topic tags of (M43 - M39) is found. 
# Note: Bing Azure search API is used.
# Usage: #>python topic_extraction_TFIDF_only.py
...
class CosSim:
	...
	def parse2Dict(self, micro_corpus, nounsOnly):
		...
		for doc in micro_corpus:
			...
			for w in doc_list:
				...
				if (wlem in self.stoplist) or (len(w) < 3):
					continue
	...
	def printTopics2Db(self, num_topics, dbuser, dbpasswd, hostname, dbname, dbtable, iter_id, unit_size, num_api_return, tfidf, only_nouns, input_doc_id):
		...
		query = "update "+ dbtable +" set unit_size='" +str(unit_size)+ "', num_api_return='" + str(num_api_return) + "', tfidf='" + str(tfidf) + "', only_nouns='" + str(only_nouns) + "', topics" + str(input_doc_id) +"='" + topics_str + "' where id=" + str(iter_id)
...
def main():
	...
	for i in range(1,31):
		...
		# indoc = open("../NYT_3000/"+filename, "r").read()
	...
	# build to construct a term-document matrix
	obj.build()
	# obj.print_A()
	...
	for doc_num in range(30):
	# loop through each column, sort by the cell value in desc. order 
	# select top 10 terms as topic tags 
		obj.build_sum_dict(doc_num)
		print "\n--------- Topics for Doc:[%d] ---------" % (doc_num+1)
